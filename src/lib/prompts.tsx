...
请紧密依据玩家的年龄、家境、智力、外貌、体质和心境，以小说化的笔法，编写一系列与玩家属性紧密相关的一句话事件。每个事件应反映出玩家在生活中的不同阶段和体验。玩家属性包括外貌、智力、财富、体质和心境，数值范围为1至10，1表示极差，5表示中等，10表示极好。请确保事件内容与玩家的属性相匹配，并符合常识，避免煽情或过度修饰，保持白描的写作风格。
在描述事件时，尝试通过具体的情境和比较来展现玩家属性的优势和劣势。例如，高智力可能帮助玩家解决复杂问题，而低财富可能让玩家面临经济压力。请确保事件之间的逻辑连贯，以展现玩家属性值的变化和其对玩家生活的影响。
在编写每个事件时，可以考虑如何将玩家的某些属性值直接与事件联系起来。例如，如果玩家的智力值很高，可能会在学术或工作上取得一些成就；如果外貌值很高，可能会受到他人的喜欢或得到某些社交优势
请将事件按照玩家年龄递增的顺序展现，每增长一岁描述一个事件，总共描述9个事件。使用“你”指代玩家。每句描述后请添加“#”作为分隔符。在描述完玩家年龄增长9岁后的事件后结束输出，最多输出十行内容。
-- 这是输出示例 --
{玩家年龄}岁: 一句话事件。#
{玩家年龄+1}岁: 一句话事件。#
{玩家年龄+2}岁: 一句话事件。#
...
export const DEATH = `
基于年龄，身体，心境，家境属性，生成一个死亡故事，给出死亡原因，并告知玩家死亡。字数在20字。
